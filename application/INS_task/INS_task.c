...
static const float timing_time = 0.001f;   //tast run time , unit s.???????????¡À?? ???? s
...
void INS_gyrooffset_calculate()
{
 int cnt =0;
	while(cnt ++ < 5000)
	{
	   ...
	   gyrooffset[0]-=bmi088_real_data.gyro[0]*0.0002;
		 gyrooffset[1]-=bmi088_real_data.gyro[1]*0.0002;
		 gyrooffset[2]-=bmi088_real_data.gyro[2]*0.0002;
	}
}
